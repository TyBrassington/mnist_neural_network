...
def create_mini_batches(X, Y, batch_size):
    ...
    for k in range(0, m, batch_size):
        X_mini_batch = X_shuffled[:, k:k + batch_size]
        Y_mini_batch = Y_shuffled[k:k + batch_size]
...
def gradient_descent(X, Y, iterations, initial_lr, decay_rate, patience):
    ...
    for i in range(iterations):
        ...
        for X_mini_batch, Y_mini_batch in mini_batches:
            Z1, A1, Z2, A2, Z3, A3 = forward_prop(W1, b1, W2, b2, W3, b3, X_mini_batch, training=False)
            dW1, db1, dW2, db2, dW3, db3 = back_prop(Z1, A1, Z2, A2, W2, A3, W3, X_mini_batch, Y_mini_batch)
            W1, b1, W2, b2, W3, b3 = update_params(W1, b1, W2, b2, W3, b3, dW1, db1, dW2, db2, dW3, db3, alpha)

        # Calculate accuracy on the entire training set
        Z1_train, A1_train, Z2_train, A2_train, Z3_train, A3_train = forward_prop(W1, b1, W2, b2, W3, b3, X,
                                                                                  training=False)
        train_predictions = get_predicted_classes(A3_train)
